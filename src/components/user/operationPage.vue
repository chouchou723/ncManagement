  ...
  <div id="operationPage">
    <div class="search-wrap">
      ...
      <el-select style="width:190px;" placeholder="请选择操作类型" filterable clearable v-model="applyType"
        @change="getVmList('', 1)" v-if="localRole==='admin'">
        <el-option v-for="(item,index) in typeList" :key="index" :value="item.type" :label="item.type">
          {{item.type}}</el-option>
      </el-select>
      ...
      <el-select style="width:190px;" placeholder="请选择用户" filterable clearable v-model="applyState"
        @change="getVmList('', 1)" v-if="localRole==='admin'">
