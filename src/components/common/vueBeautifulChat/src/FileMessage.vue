  ...
  <div class='sc-message--file' :style="messageColors">
    <div class='sc-message--file-icon'>
      ...
      <a  @click="getImg"  href="#">
        ...
        <img :src="data.file.url?data.file.url :require('./assets/file.svg')" alt='generic file icon' height="60" id='img'/>
      </a>
    </div>
  ...
  methods:{
      getImg(){
          if(this.data.file.url){
              this.$store.commit('getImg',this.data.file.url)
          }else{
              this.$message.error('暂不支持该文件打开')
          }
      },
  }
}
