<template>
  <div id="deskConfig">
    <div class="search-wrap">
      <el-input placeholder="请输入商品名称" style="width:185px;" clearable v-model="applyUser"  @keyup.enter.native="clearData" @clear="clearData(1)">
      </el-input>
    ...
    <div class="table-wrap" id="table-wrap">
      <el-table :data="tableData" v-loading="vmTableLoadingState" :height="tableHeight" @select="checkSelect" stripe
        ...
        <el-table-column prop="company" label="所属管理员" width="120">
          <template slot-scope="scope">
            <el-button v-if="scope.row.leftMer>'0'" @click="assignDesk(scope.row)" type="text"
              style="padding:0;color:#42b983">分配配额</el-button>
      ...
      <el-scrollbar wrap-class="scrollbar-wrapper" id="menuList123" ref="myScrollbar">
        <el-form :model="aform" :rules="rules2" ref="aform">
          ...
          <el-form-item label="选择模板：" :label-width="formLabelWidth" prop="template">
            <el-select style="width:190px;" placeholder="请选择选择模板" v-model="aform.template"
              :style='{width:inputLabelWidth}'>
          ...
          <el-form-item label="系统盘：" :label-width="formLabelWidth" prop="sysDataStores">
            ...
            <el-table :data="tableDataSys" style="width: 100%" :header-cell-style="{'text-align':'center'}"
              :cell-style="{'text-align':'center'}" @select="checkSelectSys" @row-click="rowClickSys" ref="multipleTableSys">
              <el-table-column type="selection" width="55">
              </el-table-column>
              ...
              <el-table-column label="剩余容量(GB)" prop="freeSizeGB">
              </el-table-column>
          ...
          <el-form-item label="用户盘：" :label-width="formLabelWidth" prop="userDataStores">
            ...
            <el-table :data="tableDataUser" style="width: 100%" :header-cell-style="{'text-align':'center'}"
              :cell-style="{'text-align':'center'}" @select="checkSelectUser" @row-click="rowClickUser" ref="multipleTableUser">
              <el-table-column type="selection" width="55">
              </el-table-column>
              <el-table-column label="id" prop="id">
              </el-table-column>
          ...
          <el-form-item label="用户盘(GB)：" :label-width="formLabelWidth" prop="disk">
            <el-input-number v-model="aform.disk" controls-position="right" :min="1">
            </el-input-number>
          </el-form-item>
          <el-form-item label="系统盘(GB)：" :label-width="formLabelWidth">
            <el-input :value="aform.template.split('|')[2]" readonly class="templateInput">
            </el-input>
    ...
    <el-dialog title="云管理员列表" :visible.sync="transformForm" :close-on-click-modal="no"
      ...
      <div class="table-wrap" id="table-wrapTran">
        <el-table :data="userListData" height="360" ref="multipleTableTran" stripe
          ...
          <el-table-column prop="phone" label="已分配配额数">
                <template slot-scope="scope">
               <el-input-number size="small" v-model="num3" :min='0'></el-input-number>
                </template>
          </el-table-column>
      ...
      <div class="table-wrap" id="table-wrapTranS">
        <el-table :data="userListDataS" height="360" ref="multipleTableTranS" stripe
          ...
          <el-table-column prop="phone" label="已分配配额数">
                 <template slot-scope="scope">
               <el-input-number size="small" v-model="num3" :min='0'></el-input-number>
              <el-button @click="assignMercount(scope.row)" type="text"  style="margin-left:10px;padding:0" v-if="num3!=10">确认</el-button>
              <el-button @click="num3=10" type="text" style="color:#f10000;padding:0" v-if="num3!=10">取消</el-button>
  ...
  export default {
    data() {
      ...
      var limitPrefix = (rule, value, callback) => {
        ...
        if (value && isZh) {
          callback('桌面前缀不能是中文')
        } else if (value && txt.length > 5) {
          callback('桌面前缀不能超过5个字符')
        } else {
          callback()
        }
      }
    ...
    methods: {
        ...
        httpAjax(url, {
          ...
          if (res) {
            this.templateList = res.map(item => {
              return {
                value: item.templateId + '|' + item.diskList[0].datastoreId + '|' + item.diskList[0]
                  .quantityGB + '|' + item.templateName,
      ...
      dispatchCloud() { //确认 修改管理员配额
        ...
        let params = {
          merIds: [data.merId],
          cloudIds: data.cloudId,
          merCount: data.merCount,
