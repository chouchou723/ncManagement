<template>
  <div id="cloudRecycle" v-loading.fullscreen="loadingState" :element-loading-text="text">
    <div class="search-wrap">
      ...
      <el-select style="width:190px;" placeholder="请选择运行状态" clearable v-model="applyState"
        @keyup.enter.native="clearData" @change="change(3)">
      ...
      <el-select style="width:190px" placeholder="请选择登录状态" clearable v-model="applyLoginState"
        @keyup.enter.native="clearData" @change="change(4)">
        <el-option v-for="(item,index) in selectLoginState" :key="index" :value="item.value" :label="item.label">
          {{item.label}}</el-option>
      </el-select>
      <el-button icon="el-icon-search" circle @click="searchData"></el-button>
      <!-- <el-button  style="float:right;margin-right:10px;" @click="handleClick(multSelection[0])" type="primary" 
    ...
    <div class="table-wrap" id="table-wrap">
      <el-table ref="multipleTable" :data="tableData" v-loading="vmTableLoadingState" :height="tableHeight" stripe
        @select="checkSelect" :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}">
        ...
        <el-table-column label="操作" width="230">
          <template slot-scope="scope">
            ...
            <!-- <el-button @click="handleClick(scope.row)" type="primary" size="small" :loading="btnLoading&&(btnIndex===scope.row.id)">恢复桌面</el-button> -->
          </template>
        </el-table-column>
        ...
        <el-table-column prop="computerName" label="桌面名称" min-width="180">
         ...
         <!-- <el-table-column prop="operatorAccount" label="所属管理员" width="100">
             <template slot-scope="scope">
                 {{scope.row.operatorAccount||'(FA同步桌面)'}}
          </template>
         </el-table-column> -->
        ...
        <!-- <el-table-column label="操作" width="100" v-if="tableData.length">
          <template slot-scope="scope">
            <el-button @click="handleClick(scope.row)" type="text" :loading="btnLoading&&(btnIndex===scope.row.id)">
              {{btnLoading&&(btnIndex===scope.row.id)?'恢复中':'恢复桌面'}}</el-button>
          </template>
        </el-table-column> -->
      </el-table>
  ...
  export default {
    ...
    //   },
      ...
      getVmList(first, page) {
        ...
        const url = `desktop/recycleBinList?${Math.random()}`
