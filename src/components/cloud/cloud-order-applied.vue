  ...
  <div id="cloudOrderApplied">
    ...
    <div class="table-wrap" id="table-wrap">
      <el-table :data="tableData" v-loading="vmTableLoadingState" :height="tableHeight" @select="checkSelect" stripe
        ...
        <el-table-column prop="name" label="桌面名称">
          <template slot-scope="scope">
            <el-button @click="handleClick(scope.row)" type="text">{{scope.row.computerName||scope.row.name}}
            </el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="feature" label="CPU/内存/磁盘" width="150">
          <template slot-scope="scope">
            <div>
                {{scope.row.feature?scope.row.feature:scope.row.cpu+'/'+scope.row.memory+'GB/'+scope.row.disk+'GB'}}
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog title="命名规则" :visible.sync="dialogFormVisibleAudit" :close-on-click-modal="no"
      ...
      <el-form :model="aform" :rules="rule2" ref="aform">
        <el-form-item label="命名规则：" :label-width="formLabelWidth" prop="namePolicy" v-if="!postfix">
         ...
         <el-form-item label="指定桌面命名后缀:" label-width="150px" v-if="postfix">
          <div>{{postfix}}</div>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getVmList(first, page) {
        ...
        const url = `desktop/desktopWaitAudit?${Math.random()}`
